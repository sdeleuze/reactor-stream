 ...
 * @since 2.5
 */
final class StreamMulticast<T, U> extends ConnectableStream<U> implements Receiver, Producer {

	final Publisher<T>												 source;
	final Supplier<? extends Processor<? super T, ? extends T>>		processorSupplier;
	final Function<Stream<T>, ? extends Publisher<? extends U>> selector;

	volatile State<T, U> connection;
	@SuppressWarnings("rawtypes")
	static final AtomicReferenceFieldUpdater<StreamMulticast, State> CONNECTION =
			AtomicReferenceFieldUpdater.newUpdater(StreamMulticast.class, State.class, "connection");

	StreamMulticast(Publisher<T> source,
			Supplier<? extends Processor<? super T, ? extends T>> processorSupplier,
			Function<Stream<T>, ? extends Publisher<? extends U>> selector) {
