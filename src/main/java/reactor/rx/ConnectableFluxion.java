 ...
 * @since 2.5
 */
public abstract class ConnectableFluxion<T> extends Fluxion<T> {

	/**
	 * Connects this {@link ConnectableFluxion} to the upstream source when the first {@link org.reactivestreams.Subscriber}
	 * subscribes.
	 *
	 * <p>
	 ...
	 *
	 * @return a {@link Fluxion} that connects to the upstream source when the first {@link org.reactivestreams.Subscriber} subscribes
	 */
	public final Fluxion<T> autoConnect() {
		return autoConnect(1);
	}

	/**
	 * Connects this {@link ConnectableFluxion} to the upstream source when the specified amount of
	 ...
	 *
	 * @return a {@link Fluxion} that connects to the upstream source when the given amount of Subscribers subscribe
	 */
	public final Fluxion<T> autoConnect(int minSubscribers) {
	 ...
	 * <img width="500" src="https://raw.githubusercontent.com/reactor/projectreactor.io/master/src/main/static/assets/img/marble/autoconnect.png" alt="">
	 *
	 * @return a {@link Fluxion} that connects to the upstream source when the given amount of subscribers subscribed
	 */
	public final Fluxion<T> autoConnect(int minSubscribers, Consumer<? super Runnable> cancelSupport) {
		if (minSubscribers == 0) {
			connect(cancelSupport);
			return this;
		}
		return new FluxionAutoConnect<>(this, minSubscribers, cancelSupport);
	}

	/**
	 * Connect this {@link ConnectableFluxion} to its source and return a {@link Runnable} that
	...
	/**
	 * Connects this {@link ConnectableFluxion} to its source and sends a {@link Runnable} to a callback that
	 * can be used for disconnecting.
	 *
	 ...
	 *
	 * @return a reference counting {@link Fluxion}
	 */
	public final Fluxion<T> refCount() {
		return refCount(1);
	}
	 ...
	 * @return a reference counting {@link Fluxion}
	 */
	public final Fluxion<T> refCount(int minSubscribers) {
		return new FluxionRefCount<>(this, minSubscribers);
	}
