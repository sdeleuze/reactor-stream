 ...
 * @since 2.5
 */
final class FluxionBuffer<T, C extends Collection<? super T>> extends FluxionSource<T, C> {
	...
	public FluxionBuffer(Publisher<? extends T> source, int size, Supplier<C> bufferSupplier) {
		this(source, size, size, bufferSupplier);
	}

	public FluxionBuffer(Publisher<? extends T> source, int size, int skip, Supplier<C> bufferSupplier) {
		super(source);
		if (size <= 0) {
