...
/**
 *
 * A connecting {@link Fluxion} Publisher (right-to-left from a composition chain perspective)
 *
 ...
 * @since 2.5
 */
public class FluxionSource<I, O> extends Fluxion<O>
		implements Receiver,
	...
	/**
	 * Unchecked wrap of {@link Publisher} as {@link Fluxion}, supporting {@link Fuseable} sources
	 *
	 * @param source the {@link Publisher} to wrap 
	 * @param <I> input upstream type
	 * @return a wrapped {@link Fluxion}
	 */
	public static <I> Fluxion<I> wrap(Publisher<? extends I> source){
		if(source instanceof Fuseable){
			return new FuseableFluxionSource<>(source);
		}
		return new FluxionSource<>(source);
	}


	public FluxionSource(Publisher<? extends I> source) {
	...
	public Timer getTimer() {
		return Fluxion.class.isAssignableFrom(source.getClass()) ? ((Fluxion) source).getTimer() : super.getTimer();
	}
	...
	final static class Operator<I, O> extends FluxionSource<I, O> {
