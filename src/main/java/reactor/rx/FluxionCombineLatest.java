 ...
 * @since 2.5
 */
final class FluxionCombineLatest<T, R>
		extends Fluxion<R>
	...
	public FluxionCombineLatest(Publisher<? extends T>[] array,
	...
	public FluxionCombineLatest(Iterable<? extends Publisher<? extends T>> iterable,
			Function<Object[], R> combiner, Supplier<? extends Queue<SourceAndArray>> queueSupplier,
					int bufferSize) {
		if (bufferSize <= 0) {
			throw new IllegalArgumentException("BUFFER_SIZE > 0 required but it was " + bufferSize);
	...
	public void subscribe(Subscriber<? super R> s) {
		...
		if (n == 1) {
			new FluxionMap<>(a[0], new Function<T, R>() {
				@Override
				public R apply(T t) {
