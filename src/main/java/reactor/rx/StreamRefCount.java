...
final class StreamRefCount<T> extends Stream<T>
	...
	static final AtomicReferenceFieldUpdater<StreamRefCount, State> CONNECTION =
			AtomicReferenceFieldUpdater.newUpdater(StreamRefCount.class, State.class, "connection");
	
	public StreamRefCount(ConnectableStream<? extends T> source, int n) {
		if (n <= 0) {
	...
	static final class State<T> implements Consumer<Runnable>, MultiProducer, Receiver {
		...
		final StreamRefCount<? extends T> parent;
		...
		public State(int n, StreamRefCount<? extends T> parent) {
			this.n = n;
			this.parent = parent;
		}
