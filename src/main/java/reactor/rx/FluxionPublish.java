...
/**
 * A connectable {@link Fluxion} which shares an underlying source and dispatches source values to subscribers in a
 ...
 * @since 2.5
 */
final class FluxionPublish<T> extends ConnectableFluxion<T>
	...
	static final AtomicReferenceFieldUpdater<FluxionPublish, State> CONNECTION =
			AtomicReferenceFieldUpdater.newUpdater(FluxionPublish.class, State.class, "connection");
	
	public FluxionPublish(Publisher<? extends T> source, int prefetch, Supplier<? extends Queue<T>> queueSupplier) {
		if (prefetch <= 0) {
	...
	static final class State<T> implements Subscriber<T>, Runnable, Receiver, MultiProducer, Backpressurable,
		...
		public State(int prefetch, FluxionPublish<T> parent) {
			this.prefetch = prefetch;
			this.parent = parent;
