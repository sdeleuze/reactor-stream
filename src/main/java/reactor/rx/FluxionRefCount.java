 ...
 * @since 2.5
 */
final class FluxionRefCount<T> extends Fluxion<T>
		implements Receiver, Loopback {
	
	final ConnectableFluxion<? extends T> source;
	
	final int n;

	volatile State<T> connection;
	@SuppressWarnings("rawtypes")
	static final AtomicReferenceFieldUpdater<FluxionRefCount, State> CONNECTION =
			AtomicReferenceFieldUpdater.newUpdater(FluxionRefCount.class, State.class, "connection");
	
	public FluxionRefCount(ConnectableFluxion<? extends T> source, int n) {
		if (n <= 0) {
	...
	static final class State<T> implements Consumer<Runnable>, MultiProducer, Receiver {
		...
		final FluxionRefCount<? extends T> parent;
		...
		public State(int n, FluxionRefCount<? extends T> parent) {
			this.n = n;
			this.parent = parent;
		}
