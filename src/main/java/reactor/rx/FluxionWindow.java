 ...
 * @since 2.5
 */
final class FluxionWindow<T> extends FluxionSource<T, Fluxion<T>> {
	...
	public FluxionWindow(Publisher<? extends T> source, int size,
	...
	public FluxionWindow(Publisher<? extends T> source, int size, int skip,
			Supplier<? extends Queue<T>> processorQueueSupplier,
	...
	@Override
	public void subscribe(Subscriber<? super Fluxion<T>> s) {
		if (skip == size) {
	...
	static final class WindowExactSubscriber<T> implements Subscriber<T>, Subscription, Runnable {
		...
		final Subscriber<? super Fluxion<T>> actual;
		...
		public WindowExactSubscriber(Subscriber<? super Fluxion<T>> actual, int size,
				Supplier<? extends Queue<T>> processorQueueSupplier) {
			this.actual = actual;
			this.size = size;
	...
	static final class WindowSkipSubscriber<T> implements Subscriber<T>, Subscription, Runnable {
		...
		public WindowSkipSubscriber(Subscriber<? super Fluxion<T>> actual, int size, int skip,
				Supplier<? extends Queue<T>> processorQueueSupplier) {
			this.actual = actual;
			this.size = size;
			this.skip = skip;
	...
	static final class WindowOverlapSubscriber<T> implements Subscriber<T>, Subscription, Runnable {
		...
		public WindowOverlapSubscriber(Subscriber<? super Fluxion<T>> actual, int size, int skip,
				Supplier<? extends Queue<T>> processorQueueSupplier,
				Queue<UnicastProcessor<T>> overflowQueue) {
			this.actual = actual;
			this.size = size;
		...
		void drain() {
			...
			final Subscriber<? super Fluxion<T>> a = actual;
