 ...
 * @since 2.5
 */
final class FluxionRetry<T> extends FluxionSource<T, T> {

	final long times;

	public FluxionRetry(Publisher<? extends T> source) {
		this(source, Long.MAX_VALUE);
	}

	public FluxionRetry(Publisher<? extends T> source, long times) {
		super(source);
		if (times < 0L) {
