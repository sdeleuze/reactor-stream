...
final class StreamPublish<T> extends ConnectableStream<T>
	...
	static final AtomicReferenceFieldUpdater<StreamPublish, State> CONNECTION =
			AtomicReferenceFieldUpdater.newUpdater(StreamPublish.class, State.class, "connection");
	
	public StreamPublish(Publisher<? extends T> source, int prefetch, Supplier<? extends Queue<T>> queueSupplier) {
		if (prefetch <= 0) {
	...
	static final class State<T> implements Subscriber<T>, Runnable, Receiver, MultiProducer, Backpressurable,
		...
		public State(int prefetch, StreamPublish<T> parent) {
			this.prefetch = prefetch;
			this.parent = parent;
