 ...
 * @since 2.5
 */
final class FluxionGroupBy<T, K, V> extends FluxionSource<T, GroupedFluxion<K, V>>
implements Fuseable, Backpressurable  {
	...
	final Supplier<? extends Queue<GroupedFluxion<K, V>>> mainQueueSupplier;

	final int prefetch;

	public FluxionGroupBy(
			Publisher<? extends T> source, 
			Function<? super T, ? extends K> keySelector,
			Function<? super T, ? extends V> valueSelector,
			Supplier<? extends Queue<GroupedFluxion<K, V>>> mainQueueSupplier,
	...
	@Override
	public void subscribe(Subscriber<? super GroupedFluxion<K, V>> s) {
		Queue<GroupedFluxion<K, V>> q;
	...
	static final class GroupByMain<T, K, V> implements Subscriber<T>,
	                                                   QueueSubscription<GroupedFluxion<K, V>>, MultiProducer, Backpressurable, Producer, Requestable,
		...
		final Subscriber<? super GroupedFluxion<K, V>> actual;

		final Queue<GroupedFluxion<K, V>> queue;
		
		final Supplier<? extends Queue<V>> groupQueueSupplier;

		final int prefetch;
		
		final ConcurrentMap<K, UnicastGroupedFluxion<K, V>> groupMap;
		...
		public GroupByMain(
				Subscriber<? super GroupedFluxion<K, V>> actual,
				Queue<GroupedFluxion<K, V>> queue,
				Supplier<? extends Queue<V>> groupQueueSupplier, 
				int prefetch,
				Function<? super T, ? extends K> keySelector,
				Function<? super T, ? extends V> valueSelector
		...
		public void onNext(T t) {
			...
			UnicastGroupedFluxion<K, V> g = groupMap.get(key);
			...
			if (g == null) {
				...
				if (cancelled == 0) {
					...
					g = new UnicastGroupedFluxion<>(key, q, this, prefetch);
					g.onNext(value);
					groupMap.put(key, g);
		...
		void signalAsyncComplete() {
			groupCount = 0;
			for (UnicastGroupedFluxion<K, V> g : groupMap.values()) {
				g.onComplete();
			}
			actual.onComplete();
			groupMap.clear();
				...
				if (GROUP_COUNT.decrementAndGet(this) == 0) {
					...
					if (!enableAsyncFusion) {
						if (WIP.getAndIncrement(this) == 0) {
							...
							GroupedFluxion<K, V> g;
							while ((g = queue.poll()) != null) {
								((UnicastGroupedFluxion<K, V>)g).cancel();
							}
		...
		void drainLoop() {
			...
			Subscriber<? super GroupedFluxion<K, V>> a = actual;
			Queue<GroupedFluxion<K, V>> q = queue;
		...
		boolean checkTerminated(boolean d, boolean empty, Subscriber<?> a, Queue<GroupedFluxion<K, V>> q) {
			if (d) {
				Throwable e = error;
		...
		@Override
		public GroupedFluxion<K, V> poll() {
			return queue.poll();
		}

		@Override
		public GroupedFluxion<K, V> peek() {
			return queue.peek();
		}

		@Override
		public boolean add(GroupedFluxion<K, V> t) {
			throw new UnsupportedOperationException("Operators should not use this method!");
		}

		@Override
		public boolean offer(GroupedFluxion<K, V> t) {
			throw new UnsupportedOperationException("Operators should not use this method!");
		}

		@Override
		public GroupedFluxion<K, V> remove() {
			throw new UnsupportedOperationException("Operators should not use this method!");
		}

		@Override
		public GroupedFluxion<K, V> element() {
			throw new UnsupportedOperationException("Operators should not use this method!");
		}
		...
		@Override
		public Iterator<GroupedFluxion<K, V>> iterator() {
			throw new UnsupportedOperationException("Operators should not use this method!");
		}
		...
		@Override
		public boolean addAll(Collection<? extends GroupedFluxion<K, V>> c) {
			throw new UnsupportedOperationException("Operators should not use this method!");
		}
	...
	static final class UnicastGroupedFluxion<K, V> extends GroupedFluxion<K, V>
	implements Fuseable, QueueSubscription<V>,
		...
		@SuppressWarnings("rawtypes")
		static final AtomicReferenceFieldUpdater<UnicastGroupedFluxion, GroupByMain> PARENT =
				AtomicReferenceFieldUpdater.newUpdater(UnicastGroupedFluxion.class, GroupByMain.class, "parent");
		...
		@SuppressWarnings("rawtypes")
		static final AtomicReferenceFieldUpdater<UnicastGroupedFluxion, Subscriber> ACTUAL =
				AtomicReferenceFieldUpdater.newUpdater(UnicastGroupedFluxion.class, Subscriber.class, "actual");
		
		volatile boolean cancelled;
		
		volatile int once;
		@SuppressWarnings("rawtypes")
		static final AtomicIntegerFieldUpdater<UnicastGroupedFluxion> ONCE =
				AtomicIntegerFieldUpdater.newUpdater(UnicastGroupedFluxion.class, "once");

		volatile int wip;
		@SuppressWarnings("rawtypes")
		static final AtomicIntegerFieldUpdater<UnicastGroupedFluxion> WIP =
				AtomicIntegerFieldUpdater.newUpdater(UnicastGroupedFluxion.class, "wip");

		volatile long requested;
		@SuppressWarnings("rawtypes")
		static final AtomicLongFieldUpdater<UnicastGroupedFluxion> REQUESTED =
				AtomicLongFieldUpdater.newUpdater(UnicastGroupedFluxion.class, "requested");
		
		volatile boolean enableOperatorFusion;

		int produced;
		
		public UnicastGroupedFluxion(K key, Queue<V> queue, GroupByMain<?, K, V> parent, int prefetch) {
			this.key = key;
			this.queue = queue;
			this.parent = parent;
