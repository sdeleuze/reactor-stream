...
*Reactor Fluxions* has the following artifacts:
...
** Not created directly, but with the Stream API (`Fluxion.map()` -> MapAction, `Fluxion.filter()` -> FilterAction...)
...
[NOTE]
Do not confuse `reactor.rx.Fluxion` with the new JDK 8 `java.util.stream.Stream`. The latter does not offer a Reactive Streams based API nor Reactive Extensions. However, the JDK 8 `Stream` API is quite complete when used with primitive types and Collections. In fact it's quite interesting for JDK 8 enabled applications to mix both JDK and Reactive Fluxion.
...
[source, java]
----
Fluxion<A> stream = Fluxion.just(a);
Fluxion<B> transformedStream = stream.map(transformationToB);

Assert.isTrue(transformationStream != stream);
stream.subscribe(subscriber1); //subscriber1 will see the data A unaltered
transformedFluxion.subscribe(subscriber2); //subscriber2 will see the data B after transformation from A.
...
[source,java]
----
Fluxion<String> st = Fluxion.just("Hello ", "World", "!"); // <1>
...
[IMPORTANT]
Cold Data Sources will be replayed from start for every fresh Subscriber passed to `Fluxion.subscribe(Subscriber)`, and, therefore, duplicate consuming is possible.
...
|Fluxion.<T, Throwable>error(*Throwable*)
|T
2+|Only emit `onError(Throwable)`.

|Fluxion.from(*Future<T>*)
|T
...
|Fluxion.from(*Iterable<T>*)
|T
2+|Emit N `onNext(T)` elements everytime `Subscription.request(N)` is invoked. If N == Long.MAX_VALUE, emit everything. Once all the array has been read, emit `onComplete()`.

|Fluxion.range(*long*, _long_)
|Long
...
|Fluxion.just(T, _T, T, T, T, T, T, T_)
|T
2+|An optimization over `Fluxion.from(Iterable)` that just behaves similarly. Also useful to emit Iterable, Array or Future without colliding with the Fluxion.from() signatures.

|Fluxion.generate(*Supplier<T>*)
|T
...
.Fluxion.concat() and Fluxion.from() in action
[source,java]
----
Processor<String,String> processor = TopicProcessor.create();

Fluxion<String> st1 = Fluxion.just("Hello "); // <1>
Fluxion<String> st2 = Fluxion.just("World "); // <1>
Fluxion<String> st3 = Fluxion.from(processor); // <2>

Fluxion.concat(st1, st2, st3) // <3>
...
<2> Decorate the core processor with `Stream` API. Note that `Fluxion.concat()` would have accepted the processor directly as a valid Publisher argument.
...
h|Fluxion.defer(*Supplier<Publisher<T>>*)
|T
2+|A lazy Publisher access using the level of indirection provided by `Supplier.get()` every time `Fluxion.subscribe(Subscriber)` is called.

h|Fluxion.generate(*BiConsumer<Long,SubscriberWithContext<T, C>*, _Function<Subscriber<T>,C>, Consumer<C>_)
|T
2+|A Stream generator with explicit callbacks for each `Subscriber` request, start and stop events. similar to `Fluxion.create(Publisher)` minus the boilerplate for common use.

h|Fluxion.switchOnNext(*Publisher<Publisher<T>>*)
|T
...
h|Fluxion.concat(*Publisher<T>*, _Publisher<T>*_)

Fluxion.concat(*Publisher<Publisher<T>>*)
|T
...
h|Fluxion.merge(*Publisher<T>, Publisher<T>*, Publisher<T>*)

Fluxion.merge(*Publisher<Publisher<T>>*)
|T
...
h|Fluxion.combineLatest(*Publisher<T1>, Publisher<T2>*, _Publisher<T3-N> x6_, *Function<Tuple2-N, C>*)
|C
...
h|Fluxion.zip(*Publisher<T1>, Publisher<T2>,* _Publisher<T3-N> x6_, *Function<Tuple2-N, C>*)
|C
...
h|Fluxion.join(*Publisher<T>, Publisher<T>*, _Publisher<T> x6_)
|List<T>
...
h|Fluxion.await(*Publisher<>*, _long, unit, boolean_)
|void
...
h|IOFluxion.<K,V>persistentMap(*String*, _deleteOnExit_)
|V
...
h|IOFluxion.<K,V>persistentMapReader(*String*)
|V
...
h|IOFluxion.decode(*Codec<SRC, IN, ?>, Publisher<SRC>*)
|IN
...
.Fluxion.create and Fluxion.defer in action
[source,java]
----
final Fluxion<String> stream1 = Fluxion.create(new Publisher<String>() {
  @Override
...
final Fluxion<String> stream2 = Fluxion.create(sub -> {
  sub.onNext("3"); // <2>
  sub.onNext("4");
  sub.onComplete();
});
...
Fluxion<String> deferred = Fluxion.defer(() -> {
...
<3> Create a `DeferredStream` that will alternate source Publisher<T> on each `Fluxion.subscribe` call, evaluating the total number of Subscribers,
...
* Using `Flux.create(args)` or `Fluxion.generate(args)` to use Functional consumers for every lifecycle step
(requested, stopped, started).
* Extending `Stream` instead of implementing `Publisher` directly to benefit from composition API

.Fluxion.generate, an alternative to create() minus some boilerplate
[source,java]
----
final Fluxion<String> stream = Fluxion.generate(
  (demand, sub) -> { // <1>
      sub.context(); // <2>
...
<3> `.subscribe()` is a "terminal" operation, which means it produces demand in Reactive Streams parlance.
<4> Publish a value into the pipeline, which will cause the tasks to be invoked.

[IMPORTANT]
Hot Data Sources will never be replayed. Subscribers will only see data from the moment they have been passed to `Fluxion.subscribe(Subscriber)`.
An exception applies for `BehaviorBroadcaster` (last emitted element is replayed); `Mono.delay()` and `Fluxion.interval()` will also maintain unique timed cursors but will still ignore backpressure.
...
h|Mono.delay(*delay*, _unit, timer_)
|N/A
|Long
3+|Start a Timer on `Fluxion.subscribe(Subscriber)` call and emit a single `onNext(0L)` then `onComplete()` once the delay is elapsed. Be sure to pass the optional argument `Timer` if there is no current active `Environment`.
`Subscription.request(long)` will be ignored as no backpressure can apply to a scheduled emission.

h|Fluxion.interval(*period*, _unit, timer_)
|N/A
|Long
3+|Start a Timer on `Fluxion.subscribe(Subscriber)` call and every period of time emit `onNext(N)` where N is an incremented counter starting from 0. Be sure to pass the optional argument `Timer` if there is no current active `Environment`.
`Subscription.request(long)` will be ignored as no backpressure can apply to a scheduled emission.

h|Fluxion.<T>switchOnNext()
|Publisher<T>
|T
...
That basically means the Reactor `Stream` user will explicitely call `Fluxion.subscribe(Subscriber)` or, alternativly, *terminal* actions such as `Fluxion.consume(Consumer)` to materialize all the registered operations.
Before that `Actions` don't really exist. We use `Fluxion.lift(Supplier)` to defer the creation of these Actions until `Fluxion.subscribe(Subscriber)` is explicitely called.
...
Fluxion<String> stream = Fluxion.just("a","b","c","d","e","f","g","h");

//prepare two unique pipelines
Fluxion<String> actionChain1 = stream.map(String::toUpperCase).filter(w -> w.equals("C"));
Fluxion<Long> actionChain2 = stream.dispatchOn(SchedulerGroup.single()).take(5).count();
...
==== Publish/Subscribe
For *Fan-Out* to subscribers from a unified pipeline, `Fluxion.subscribeWith(Subscribe)`, `Fluxion.multicast()`, `Stream
.broadcastOn()` and `Fluxion.broadcastTo()` can be used.
...
Fluxion<String> sharedStream = stream.doOnNext(System.out::println).multicast();

//prepare two unique pipelines
Fluxion<String> actionChain1 = sharedFluxion.map(String::toUpperCase).filter(w -> w.equals("C"));
Fluxion<Long> actionChain2 = sharedFluxion.take(5).count();
...
_batchConsumeOn_
|Control
2+|Similar to `consume` but will request the mapped `Long` demand given the previous demand and starting with the default `Fluxion.useCapacity()`. Useful for adapting the demand dynamically due to various factors.

h|adaptiveConsume(*Consumer<T>*, _Consumer<T>, Consumer<T>_, *Function<Fluxion<Long>,Publisher<Long>>*)

_adaptiveConsumeOn_
|Control
2+|Similar to `batchConsume` but will request the computed sequence of demand `Long`. It can be used to insert flow-control such as `Mono.delay()` to delay demand.
...
|Fluxion<T>
2+|Turn Any Stream into a *Hot* Fluxion. This will prevent pipeline duplication by immediately materializing the `Stream` and be ready to publish the signal to N Subscribers downstream.
The demand will be aggregated from all child Subscribers.

h|broadcastTo(*Subscriber<T>*)
|Subscriber<T>
2+|An alternative to `Fluxion.subscribe` which allows method chaining since the returned instance is the same as the passed argument.
...
To limit the amount of data in-flight in a `Stream`, use the link:/docs/api/reactor/rx/Fluxion.html#capacity-long-[`.useCapacity(long)`] method.
...
[TIP]
We leave as an exercise to the *Reactor User* to study the benefit of setting capacity vs computing dynamic demand with `Fluxion.adaptiveConsume` or a custom `Subscriber`.
...
** `subscribe()` synchronously propagates back which might affect stack size use. If that becomes an issue, use a
delegate `Processor` that runs `subscribe()` on a `Environment.tailRecurse()` dispatcher. Then `subscribeWith()` it at
...
To observe values, use link:/docs/api/reactor/rx/Fluxion.html#observe-java.util.function.Consumer-[.doOnNext(Consumer<? super T>)]. To observe errors without dealing with them definitively, use link:/docs/api/reactor/rx/Fluxion.html#doOnError-java.lang.Class-java.util.function.BiConsumer-[.doOnNext(Class<? extends Throwable>, BiConsumer<Object,? extends Throwable>)]. To observe the Reactive Streams complete signal, use link:/docs/api/reactor/rx/Fluxion.html#doOnComplete-java.util.function.Consumer-[.doOnComplete(Consumer<Void>)]. To observe the cancel signal, use link:/docs/api/reactor/rx/Fluxion.html#doOnCancel-java.util.function.Consumer-[.doOnCancel(Consumer<Void>)]. To observe the Reactive Streams subscribe signal, use link:/docs/api/reactor/rx/Fluxion.html#observeSubscribe-java.util.function.Consumer-[.observeSubscribe(Consumer<? super Subscription<T>>)].
...
The most popular one is the link:/docs/api/reactor/rx/Fluxion.html#filter-java.util.function.Predicate-[`.filter(Predicate<T>)`] method.
...
There are various ways to tell a Fluxion<T> its boundary in time, in size and/or on a specific condition.
The most popular one is the link:/docs/api/reactor/rx/Fluxion.html#take-long-[`.take(long)`] method.
...
The most popular transforming action is link:/docs/api/reactor/rx/Fluxion.html#map-java.util.function.Function-[.map(Function<? super I, ? extends O>)].
...
If you want to execute a distinct pipeline `Fluxion<V>` or `Publisher<V>` given an actual input data, you can use combinatory actions such as `.flatMap(Function)` and other `reactor.rx.action.combination` actions.

To transform values into a distinct, possibly asynchronous `Publisher<V>`, use link:/docs/api/reactor/rx/Fluxion.html#map-java.util.function.Function-[.flatMap(Function<? super I, ? extends Publisher<? extends O>)].
...
.Fluxion.flatMap(Function)
[source,java]
----
Stream
  .range(1, 100)
  .flatMap(number -> Fluxion.range(1, number).publishOn(SchedulerGroup.io()) ) // <1>
  .consume(
...
|It's not specific to any API, but if the current Stream is dispatched on a `SynchronousDispatcher`, it is actually blocking when a *terminal* action is starting, such as `subscribe()`.
...
By default the `Stream` is assigned with a `SynchronousDispatcher` and will inform its immediate child `Actions` via `Fluxion.getDispatcher()`.

[IMPORTANT]
Various `Stream` factories, the `Broadcaster`, the `Fluxion.dispatchOn`  and the terminal `xxxOn` methods might alter the default `SynchronousDispatcher`.

.It is fundamental to understand the three major thread switches available in Reactor Fluxion:
****
* The `Fluxion.dispatchOn` action is the only one available under `Stream` that will be dispatching *onError*, *onComplete* and *onNext* signals on the given `Dispatcher`.
...
* The `Fluxion.publishOn` action will be executing *onSubscribe* only on the passed dispatcher.
...
** The first `request` might still execute in the *onSubscribe* thread, for instance, with `Fluxion.subscribe()` actions.
* Attaching a `Processor` via `Fluxion.process` for instance can affect the thread too. The `Processor` such as `TopicProcessor` will run the `Subscribers` on its managed threads.
...
This is the default behavior for unbounded `Fluxion.consume` actions.
...
However, `publishOn` is less useful when more than 1 request will be involved, like in step-consuming with `Fluxion.useCapacity(n)`.
...
Stream
  .range(1, 100)
  .subscribeWith(TopicProcessor.create()) // <2>
  .publishOn(SchedulerGroup.io()) // <1>
  .useCapacity(1); // <3>
  .subscribe(); // <4>
----
...
* *Window* : Discrete boundaries *forwarding* `onNext(T)` into distinct `Fluxion<T>` passed to the child `Subscriber`.
** Used best with accumulators such as `reduce` or any subscriber/action reacting to `onComplete()`.
** Can be combined with `flatMap` or `concatMap` which merge back the individual windows in a common `Fluxion<T>`
****
...
.Chunk processing with Stream buffers (returning Fluxion<List<T>>):
|===
h|Fluxion<T> API
^.^a|*Role*
...
Forwarding grouped sequences of data `T` into a `Fluxion<T>` serves three main purposes:
...
* Parallelizing grouped sequences combined with `dispatchOn` for each generated `Fluxion<T>` and merging their results back.
...
[NOTE]
====
`Fluxion<T>` windows are slightly less optimized but equivalent aggregating producer than buffer API if combined with the aggregate-all `Fluxion.buffer()` method:
...
Fluxion<Integer> sensorDataStream = Fluxion.from(createTestDataset(1000));
...
.Chunk processing with Stream (returning Fluxion<Fluxion<T>>):
|===
h|Fluxion<T> API
^.^a|*Role*
...
h|window(_int_)
|Forward to a generated `Fluxion<T>` until `onComplete()` or the given `int` argument is reached which starts over a new `Stream`.

h|window(*Publisher<?>*, _Supplier<? extends Publisher<?>>_)
|Forward to a generated `Fluxion<T>` until `onComplete()` or when the first `Publisher<?>` argument emits a signal. The optional `Supplier<? extends Publisher<?>>` supplies a sequence whose first signal will end the linked aggregation. That means overlapping (sliding buffers) and disjointed aggregations can be emitted to the child `Subscriber<Fluxion<T>>`.

h|window(*Supplier<? extends Publisher<?>>*)
|Forward to a generated `Fluxion<T>`  until `onComplete()` or in coordination with a provided `Publisher<?>`. The `Supplier<? extends Publisher<?>>` supplies a sequence whose first signal will end the linked `Fluxion<T>` and start a new one immediately.

h|window(*int, int*)
|Forward to a generated `Fluxion<T>`  until `onComplete()` or the given *skip* (the second `int` argument) is reached which starts over a new `Fluxion<T>`. The *size* (the first `int` argument) will delimit the maximum number of aggregated elements by buffer. That means overlapping (sliding buffers) and disjointed sequences can be emitted to the child `Subscriber<Fluxion<T>>`.

h|window(*long*, TimeUnit, Timer_)
|Forward to a generated `Fluxion<T>` until `onComplete()` or the elapsed *period* (the `long` argument) is reached, which starts over a new `Fluxion<T>`.

h|window(*long, long*, TimeUnit, Timer_)
|Forward to a generated `Fluxion<T>`  until `onComplete()` or the given *timeshift* (the second `long` argument) is reached. The *timespan* (the first `long` argument) will delimit the maximum number of aggregated elements by buffer. That means overlapping (sliding buffers) and disjointed sequenced can be emitted to the child `Subscriber<Fluxion<T>>`.

h|window(*int, long*, _TimeUnit, Timer_)
|A combination of `buffer(int)` *OR* `buffer(long, TimeUnit, Timer)` conditions. It forwards to a generated `Fluxion<T>` until the given *size* has been reached or the *timespan* has elapsed.
...
Fluxion.just(1,2,3,4,5)
  ...
  .subscribe()
----
<1> The buffer operation set useCapacity(3)
<2> subscribe() or any downstream action is set with useCapacity(2), an implicit onOverflowBuffer() is added
...
Ultimately the `Subscriber` can request data one by one, limiting the in-flight data to one element all along the pipeline and requesting one more after each successful `onNext(T)`. The same behavior can be obtained with `useCapacity(1).consume(...)`.

[source,java]
----
Fluxion.range(1,1000000)
...
h|useCapacity(*long*)
|<<streams.adoc#stream-capacity, Set the capacity>> to this `Fluxion<T>` and all downstream actions.
...
_batchConsumeOn_
|Similar to `consume` but will request the mapped `Long` demand given the previous demand and starting with the default `Fluxion.useCapacity()`. Useful for adapting the demand from various factors.

h|adaptiveConsume(*Consumer<T>*, _Consumer<T>, Consumer<T>_, *Function<Fluxion<Long>,Publisher<Long>>*),

_adaptiveConsumeOn_
|Similar to `batchConsume` but will request the computed sequence of demand `Long`. It can be used to insert flow-control such as `Mono.delay()` to delay demand.  The `AdaptiveConsumerAction` will subscribe to the produced sequence and immediately forwards `onNext(Long)` to the upstream `request(long)`.
...
|Reduce the volume of a `Fluxion<T>` by selecting the last (or the first) `onNext(T)` signals matching the given conditions. These conditions can be timed, sized, timed or sized, and interactive (event-driven).

h|zip(_arguments_), zipWith(_arguments_)
|Reduce the volume of N `Fluxion<T>` to the least signals producing zipped `Publisher`. The aggregated signals from each `Publisher` can be used to produce a distinct value from the N most recent upstream `onNext(T)`.
...
.Fluxion.zipWith(Function)
[source,java]
----
Stream
  .range(1, 100)
  .zipWith( Fluxion.generate(System::currentTimeMillis), tuple -> tuple ) // <1>
  .consume(
...
h|Fluxion.flatMap(Function<T, Publisher<V>>)
|An <<streams.adoc#stream-flatmap,Async transformation>> is a typed shortcut for `merge(map(Function<T, Publisher<V>>))`.
...
h|Fluxion.merge(Publisher<T>, _Publisher<T> x7_)

Fluxion.merge(Publisher<Publisher<T>>)

Fluxion.mergeWith(Publisher<T>)

h|Fluxion.concat(Publisher<T>, _Publisher<T> x7_)

Fluxion.concat(Publisher<Publisher<T>>)

Fluxion.concatWith(Publisher)

Fluxion.startWith(Publisher)
...
h|Fluxion.combineLatest(Publisher<T>, _Publisher<T> x7_, Function<Tuple,V>)

Fluxion.combineLatest(Publisher<Publisher<T>>, Function<Tuple,V>)
...
h|Fluxion.zip(Publisher<T>, _Publisher<T> x7_, Function<Tuple,V>)

Fluxion.zip(Publisher<Publisher<T>>, Function<Tuple,V>)

Fluxion.zipWith(Publisher<T>, Function<Tuple2,V>)
...
h|Fluxion.join(Publisher<T>, _Publisher<T> x7_)

Fluxion.join(Publisher<Publisher<T>>)

Fluxion.joinWith(Publisher<T>)
...
** Error isolation
. Compose services with the *Reactor Fluxion* API
...
h|Fluxion.create(Publisher), Fluxion.defer(Supplier), Fluxion.from(Publisher), Fluxion.generate(Supplier)
...
h|Fluxion.publishOn(Dispatcher), Fluxion.dispatchOn(Dispatcher), _Core Processors_
...
public Fluxion<User> allFriends(final User user)  {
  return Stream
    .defer(() ->
      Fluxion.just(userDB.findAllFriends(user)))
    .timeout(3, TimeUnit.Seconds)
...
public Fluxion<User> filteredFind(String name){
    return get(name)
      .stream()
      .filter(User::isAdmin)
...
** The returned Fluxion<User> resume on the first allFriend() signal
****
...
.retryWhen( errors ->
  errors
  .zipWith(Fluxion.range(1,3), t -> t.getT2())
  .flatMap( tries -> Mono.delay(tries) )
)
.consume(System.out::println);
----
2+^.^h|The Result
...
****
* `Fluxion.merge(source1, source2)` is a non-blocking coordinating operation mixing the two queries in one
* `buffer()` will aggregate all results until completion or error (which we timed previously)
* `retryWhen(Function<Fluxion<Throwable>, Publisher<?>>` will keep re-subscribing if an error is propagated
** `zipWith` will combine errors with a number of tries up to 3 times
** `zipWith` only return the number of tries from the tuple
** `flatMap` + `Mono.delay(long)` convert each try into a delayed signal (seconds by default)
...
* `Fluxion.generate` is a `Flux` which intercepts requests, start and stop.
...
* The Consuming aspect can start using tools such as `useCapacity(long)` or `buffer(int)` to batch consume the request 5 by 5.
...
In the end, the last `Subscriber` in the chain will be notified with the `onError(t)` callback method. If that `Subscriber` is a `ConsumerAction` for instance, Reactor will re-route an error if no _errorConsumer_ callback has been assigned using `Fluxion.consume(dataConsumer, errorConsumer)`. The route will trigger the current `Environment` error journal if set, which by default uses SLF4J to log errors.
...
Promise<List<String>> promise =
    broadcaster
        .timeout(1, TimeUnit.SECONDS, Fluxion.fail(new Exception("another one!"))) // <1>
        .onErrorResumeNext(Fluxion.just("Alternative Message")) // <2>
        .toList();
...
h|retryWhen(Function<Fluxion<Throwable>,Publisher<?>>)
...
h|Fluxion.onOverflowBuffer(CompletableQueue)
|

h|IOFluxion.persistentMapReader()
|

h|IOFluxion.persistentMap()
|
...
h|Fluxion<T> API or Factory method
|Output Type
2+|Role
