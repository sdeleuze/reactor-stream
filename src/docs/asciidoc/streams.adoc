...
Stream.from(aListOfString)
  ...
  .flatMap(words ->
    BiStream.reduceByKey(words, (prev, next) -> prev + next)
...
h|Stream.createWith(*BiConsumer<Long,SubscriberWithContext<T, C>*, _Function<Subscriber<T>,C>, Consumer<C>_)
|T
...
h|BiStream.reduceByKey(*Publisher<Tuple2<KEY,VALUE>>*, _Map<KEY,VALUE>, Publisher<MapStream.Signal<KEY, VALUE>>_, *BiFunction<VALUE, VALUE, VALUE>*)
|Tuple2<KEY,VALUE>
...
h|BiStream.scanByKey(*Publisher<Tuple2<KEY,VALUE>>*, _Map<KEY,VALUE>, Publisher<MapStream.Signal<KEY, VALUE>>_, *BiFunction<VALUE, VALUE, VALUE>*)
|Tuple2<KEY,VALUE>
...
* Using `Flux.create(args)` or `Stream.createWith(args)` to use Functional consumers for every lifecycle step
(requested, stopped, started).
* Extending `Stream` instead of implementing `Publisher` directly to benefit from composition API

.Stream.createWith, an alternative to create() minus some boilerplate
[source,java]
----
final Stream<String> stream = Stream.createWith(
  (demand, sub) -> { // <1>
      sub.context(); // <2>
...
Stream
  .range(1, 100)
  .flatMap(number -> Stream.range(1, number).subscribeOn(Environment.workDispatcher()) ) // <1>
  .consume(
...
* `Stream.createWith` is a `Flux` which intercepts requests, start and stop.
...
Promise<Long> result = source
    ...
    .nest() // <3>
    .flatMap(self ->
            BiStream.reduceByKey(self, (prev, next) -> prev + 1) // <4>
    )
...
h|BiStream.reduceByKey()
|
2+|

h|BiStream.scanByKey()
|
2+|
