...
_batchConsumeOn_
|Control
2+|Similar to `consume` but will request the mapped `Long` demand given the previous demand and starting with the default `Fluxion.capacity()`. Useful for adapting the demand dynamically due to various factors.
...
To limit the amount of data in-flight in a `Stream`, use the link:/docs/api/reactor/rx/Fluxion.html#capacity-long-[`.capacity(long)`] method.
...
However, `publishOn` is less useful when more than 1 request will be involved, like in step-consuming with `Fluxion.capacity(n)`.
...
<2> subscribe() or any downstream action is set with capacity(2), an implicit onOverflowBuffer() is added
...
_batchConsumeOn_
|Similar to `consume` but will request the mapped `Long` demand given the previous demand and starting with the default `Fluxion.capacity()`. Useful for adapting the demand from various factors.
