...
An exception applies for `BehaviorBroadcaster` (last emitted element is replayed); `Mono.delay()` and `Stream.interval()` will also maintain unique timed cursors but will still ignore backpressure.
...
h|Stream.interval(*period*, _unit, timer_)
|N/A
|Long
...
==== Publish/Subscribe
For *Fan-Out* to subscribers from a unified pipeline, `Stream.subscribeWith(Subscribe)`, `Stream.multicast()`, `Stream
.broadcastOn()` and `Stream.broadcastTo()` can be used.
...
** The first `request` might still execute in the *onSubscribe* thread, for instance, with `Stream.subscribe()` actions.
...
****
* `Stream.merge(source1, source2)` is a non-blocking coordinating operation mixing the two queries in one
