...
Publisher<String> fileStream = new Publisher<String>() { // <1>
	...
	public void subscribe(final Subscriber<? super String> subscriber) {
		...
		try {
			...
			Streams.<String, FileNotFoundException> error(e)
...
.Obviously there are ways to make that code more efficient but also do more with less. Let's take a look at Reactor alternative `PublisherFactory` from core module, which is used by `Streams.generate()` methods from stream module as well.
...
We can use `Flux`, or <<streams.adoc#streams-basics, Streams factories>> like `Streams.generate()` to quickly achieve common use cases:
