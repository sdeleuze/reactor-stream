...
Publisher<String> fileStream = new Publisher<String>() { // <1>
	...
	public void subscribe(final Subscriber<? super String> subscriber) {
		...
		try {
			...
			Fluxion.<String, FileNotFoundException> error(e)
				.subscribe(subscriber); // <8>
		}
	}
};

Fluxion.from(fileStream)
	.capacity(4L) // <9>
	.consumeOn( // <10>
...
.Obviously there are ways to make that code more efficient but also do more with less. Let's take a look at Reactor alternative `PublisherFactory` from core module, which is used by `Fluxion.generate()` methods from stream module as well.
...
Flux<String> fileStream = Flux.create(
	(n, sub) -> { // <1>
		String line;
	...
	inputStream -> inputFluxion.close() // <6>
);
...
We can use `Flux`, or <<streams.adoc#streams-basics, Streams factories>> like `Fluxion.generate()` to quickly achieve common use cases:
...
In this other exercise, we will focus more on the composition power in your hands with *Reactor Fluxion* module. A classic use-case is to build self-healing data pipelines using the http://martinfowler.com/bliki/CircuitBreaker.html[Circuit Breaker Pattern] (maybe soon available in `Stream` API, maybe).
...
final Fluxion<String> openCircuit = Fluxion.just("Alternative Message"); // <2>

final StreamProcessor<Publisher<? extends String>, String> circuitSwitcher = Fluxion.switchOnNext(); // <3>
		...
		circuitSwitcher // <6>
			...
			.doOnSubscribe(s -> { // <8>
				...
				if (failures.compareAndSet(maxErrors, 0)) {
					...
					Mono
						.delay(1)  // <10>
						.subscribe(Subscribers.consumer(ignore -> circuitSwitcher.onNext(closeCircuit)));
				}
			})
			.retry() // <11>
			.toList(); // <5>
...
<3> Create a hot Switch On Next `Processor` accepting new `Publisher` to consume data from.
<4> Prepare shared counters for successes and failures.
<5> Return a `Mono` from `Fluxion.toList()` to convert the `Stream` into an eventual `List`
...
NetFluxion.<String, String>httpServer(spec ->
		spec
...
keyboardFluxion.onNext(KeyEvent.VK_RIGHT);
keyboardFluxion.onNext(KeyEvent.VK_UP);
keyboardFluxion.onNext(KeyEvent.VK_UP);
keyboardFluxion.onNext(KeyEvent.VK_DOWN);
keyboardFluxion.onNext(KeyEvent.VK_DOWN);
keyboardFluxion.onNext(KeyEvent.VK_LEFT);
keyboardFluxion.onNext(KeyEvent.VK_RIGHT);
keyboardFluxion.onNext(KeyEvent.VK_LEFT);
keyboardFluxion.onNext(KeyEvent.VK_RIGHT);
keyboardFluxion.onNext(KeyEvent.VK_B);
keyboardFluxion.onNext(KeyEvent.VK_A);
keyboardFluxion.onNext(KeyEvent.VK_C);
keyboardFluxion.onComplete();

System.out.println(konamis.get());
----
