...
.Obviously there are ways to make that code more efficient but also do more with less. Let's take a look at Reactor alternative `PublisherFactory` from core module, which is used by `Stream.generate()` methods from stream module as well.
...
final StreamProcessor<Publisher<? extends String>, String> circuitSwitcher = Stream.switchOnNext(); // <3>
...
<5> Return a `Mono` from `Stream.toList()` to convert the `Stream` into an eventual `List`
