...
public class FluxionTests extends AbstractReactorTest {
	...
	public void testComposeFromSingleValue() throws InterruptedException {
		Fluxion<String> stream = Fluxion.just("Hello World!");
		Fluxion<String> s = stream.map(s1 -> "Goodbye then!");
	...
	public void testComposeFromMultipleValues() throws InterruptedException {
		Fluxion<String> stream = Fluxion.just("1", "2", "3", "4", "5");
		Fluxion<Integer> s = stream.map(STRING_2_INTEGER)
	...
	public void testMerge() throws InterruptedException {
		Fluxion<String> stream1 = Fluxion.just("1", "2");
	...
	public void testFirstAndLast() throws InterruptedException {
		Fluxion<Integer> s = Fluxion.fromIterable(Arrays.asList(1, 2, 3, 4, 5));

		Fluxion<Integer> first = s.everyFirst(5);
		Fluxion<Integer> last = s.every(5);
	...
	public void testHandlersErrorsDownstream() throws InterruptedException {
		Fluxion<String> stream = Fluxion.just("1", "2", "a", "4", "5");
	...
	public void mapManyFlushesAllValuesThoroughly() throws InterruptedException {
		...
		Fluxion<Integer> tasks = d.dispatchOn(asyncGroup)
		                          .partition(8)
	...
	<T> void await(Fluxion<T> s, Matcher<T> expected) throws InterruptedException {
		await(1, s, expected);
	}
	...
	public void konamiCode() throws InterruptedException {
		...
		Mono<List<Boolean>> konamis = Fluxion.fromProcessor(keyboardStream.start())
	...
	public void failureToleranceTest() throws InterruptedException {
		...
		final Fluxion<String> openCircuit = Fluxion.just("Alternative Message");

		final FluxionProcessor<Publisher<? extends String>, String> circuitSwitcher = Fluxion.switchOnNext();
	...
	public void partitionByHashCodeShouldNeverCreateMoreStreamsThanSpecified() throws Exception {
		Fluxion<Integer> stream = Fluxion.range(-10, 20)
	...
	public void shouldNotFlushStreamOnTimeoutPrematurely() throws Exception {
		...
		FluxionProcessor<Integer, Integer> batchingStreamDef = Broadcaster.async(asyncGroup);
	...
	public void zipOfNull() {
		try {
			Fluxion<String> as = Fluxion.just("x");
			Fluxion<String> bs = Fluxion.just((String)null);

			assertNull(Fluxion.zip(as, bs).next().get());
		}
	...
	public void shouldCorrectlyDispatchComplexFlow() throws InterruptedException {
		...
		Fluxion<Integer> s = Fluxion.just("2222")
		                            .map(Integer::parseInt)
		                            .flatMap(l -> Fluxion.<Integer>merge(globalFeed.dispatchOn(asyncGroup),
				                           Fluxion.just(1111, l, 3333, 4444, 5555, 6666)).log("merged")
		                                                                                 .dispatchOn(asyncGroup)
		                                                                                 .log("dispatched")
	...
	public void testParallelWithJava8StreamsInput() throws InterruptedException {
		...
		Fluxion<Integer> worker = Fluxion.range(0, max)
		                                 .dispatchOn(asyncGroup);
		worker.partition(2)
	...
	public void testBeyondLongMaxMicroBatching() throws InterruptedException {
		...
		Fluxion<Integer> worker = Fluxion.fromIterable(tasks)
		                                 .log("before")
		                                 .dispatchOn(asyncGroup);
	...
	public void testDiamond() throws InterruptedException, IOException {
		...
		Fluxion<Point> points = Flux.<Double, Random>create(sub -> sub.onNext(sub.context()
		                                                                         .nextDouble()),
				sub -> new Random()).as(Fluxion::from)
		                            .log("points")
		                             ...
		                             .as(Fluxion::from); //.multicast(); works because no async boundary

		Fluxion<InnerSample> innerSamples = points.log("inner-1")
		                                          .filter(Point::isInner)
		                                          .map(InnerSample::new)
		                                          .log("inner-2");

		Fluxion<OuterSample> outerSamples = points.log("outer-1")
		                                          .filter(p -> !p.isInner())
		                                          .map(OuterSample::new)
		                                          .log("outer-2");

		Fluxion.merge(innerSamples, outerSamples)
		       .dispatchOn(asyncGroup)
	...
	public void shouldWindowCorrectly() throws InterruptedException {
		Fluxion<Integer> sensorDataStream = Fluxion.fromIterable(createTestDataset(1000));
	...
	public void shouldThrottleCorrectly() throws InterruptedException {
		Fluxion.range(1, 10000000)
		       ...
		       .requestWhen(reqs -> reqs.flatMap(req -> {
			       ...
			       return Fluxion.range(1, (int) numBatches)
			                     .map(x -> batchSize);
		       }))
		       .subscribe();
	}
	...
	public void consistentMultithreadingWithPartition() throws InterruptedException {
		...
		InterruptableSubscriber<?> c = Fluxion.range(1, 10)
		                                      .groupBy(n -> n % 2 == 0)
...
// code by @masterav10
	...
	public void combineWithOneElement() throws InterruptedException, TimeoutException {
		...
		Fluxion<Object> s1 = Broadcaster.replayLastOrDefault(new Object())
		                                .dispatchOn(asyncGroup);
		Fluxion<Object> s2 = Broadcaster.replayLastOrDefault(new Object())
		                                .dispatchOn(asyncGroup);

		// The following works:
		//List<Fluxion<Object>> list = Arrays.asList(s1);
		// The following fails:
		List<Fluxion<Object>> list = Arrays.asList(s1, s2);

		Fluxion.combineLatest(list, t -> t)
		       .log()
		       .doOnNext(obj -> {
			       ref.set(obj);
			       phaser.arrive();
	...
	/**
	 * This test case demonstrates a silent failure of {@link Fluxion#interval(long)} when a resolution is specified that
	...
	public void multiplexUsingProcessors() throws Exception {
		...
		final Fluxion<Integer> forkStream = Fluxion.just(1, 2, 3)
		                                           .log("begin-computation");
		final Fluxion<Integer> forkStream2 = Fluxion.just(1, 2, 3)
		                                            .log("begin-persistence");

		final TopicProcessor<Integer> computationBroadcaster = TopicProcessor.create("computation", BACKLOG);
		final Fluxion<String> computationStream = Fluxion.fromProcessor(computationBroadcaster)
		                                                 .map(i -> Integer.toString(i));

		final TopicProcessor<Integer> persistenceBroadcaster = TopicProcessor.create("persistence", BACKLOG);
		final Fluxion<String> persistenceStream = Fluxion.fromProcessor(persistenceBroadcaster)
		                                                 .map(i -> "done " + i);
		...
		final Fluxion<List<String>> joinStream =
				Fluxion.join(computationStream.log("log1"), persistenceStream.log("log2"));
	...
	public void multiplexUsingDispatchersAndSplit() throws Exception {
		...
		final Fluxion<List<String>> computationStream =
		...
		final Fluxion<List<String>> persistenceStream =
		...
		Fluxion<Integer> forkStream = forkBroadcaster.dispatchOn(SchedulerGroup.single("fork", BACKLOG))
		                                             .log("fork");

		forkStream.subscribe(computationBroadcaster);
		forkStream.subscribe(persistenceBroadcaster);

		final Fluxion<List<String>> joinStream = Fluxion.join(computationStream, persistenceStream)
		...
		final Mono<List<String>> listPromise = joinStream.flatMap(Fluxion::fromIterable)
		                                                 .log("resultStream")
		                                                 .toList()
